...
const seed = async () => {
    try {
        ...
        for(let i = 0; i < reviewList.length; i++) {
            await Review.create({ ...reviewList[i], userId: random(1, userList.length - 1 ), itemId: random(1, itemsList.length - 1) })
        }
        ...
        for(let orderId = 1; orderId <= orders.length; orderId++) {
            ...
            for (let j = 0; j < numberOfItems; j++){
                await OrderItems.create({ quantity: random(1, 10), priceOrdered: 100.00, orderId: orderId, itemId: itemIds[j] })
            }
        }
